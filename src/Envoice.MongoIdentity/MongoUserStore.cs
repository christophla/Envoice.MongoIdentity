...
using Envoice.MongoIdentity.Helpers;
...
namespace Envoice.MongoIdentity.MongoDB
{
      ...
      where TUser : MongoIdentityUser
  {
    ...
    public MongoUserStore(string connectionString) : this(Util.GetDatabaseFromSqlStyle(connectionString))
    {
    }

    public MongoUserStore(IMongoDatabase database) : this(database, Constants.DefaultCollectionName)
    {
    }
    ...
    public MongoUserStore(IMongoDatabase database, string userCollectionName)
    {
      ...
      Condition.Requires(userCollectionName, "userCollectionName").IsNotNullOrWhiteSpace();

      _usersRepository = new MongoRepository<TUser>(database, userCollectionName);
      _usersRepositoryManager = new MongoRepositoryManager<TUser>(database, userCollectionName);
